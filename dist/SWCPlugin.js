...
class SWCPlugin {
  ...
  async __transformHtml(context, filePath, loader) {
    ...
    const inlineScripts = dom5_1.queryAll(
      documentAst,
      dom5_1.predicates.AND(
        dom5_1.predicates.hasTagName('script'),
        dom5_1.predicates.NOT(dom5_1.predicates.hasAttr('src')),
        dom5_1.predicates.OR(
          dom5_1.predicates.NOT(dom5_1.predicates.hasAttr('type')),
          dom5_1.predicates.hasAttrValue('type', 'module'),
        ),
      ),
    );
      ...
      jsc: {
        ...
        transform: {
          react: {
            ...
            pragmaFrag: this.swcConfig.jsxFragment,
          },
        },
  ...
  async __transformCode(code, filePath, loader) {
    try {
      ...
      const { code: transformedCode } = await core_1.transform(code, transformOptions);
      return transformedCode;
    } catch (e) {
      if (Array.isArray(e.errors)) {
        const msg = e.errors[0];
        if (msg.location) {
          throw new dev_server_core_1.PluginSyntaxError(
            msg.text,
            filePath,
            code,
            msg.location.line,
